...
#thread for tfwarn function
def threadTf():
   exwarn.configure(text  = 'Only 1 file can be selected')
   exwarn.place(x = 116, y = 130)
   time.sleep(2)
   exwarn.place(x = 1000, y = 1000)
...
def faceVd():
   global ln,uI2,x,y,w,h,faces,ch, imgtk, video,cap, fca, result, cc, out, state, tst, imgtk
   try:
      exitBtn.place(x = 1000, y =1000)
      face_cascade_db = cv2.CascadeClassifier(cv2.data.haarcascades+ 'haarcascade_frontalface_default.xml')
...
#func for closing
def close():
   global sm,p,cc,chch,Aquit, result, tst
